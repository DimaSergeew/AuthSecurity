...
captcha:
  ...
  web-bind: "0.0.0.0"             # см. ниже — для прод поставь "127.0.0.1"
...
После правки `config.yml` параметры `enabled`, `web-bind`, `web-port` и
...
Если nginx стоит на той же машине, плагин по умолчанию слушает на
`0.0.0.0`, и теоретически доступен напрямую по `http://ip:25590`.
Самый надёжный способ — закрыть его на уровне сокета. В `config.yml`:
...
Перезапусти сервер — теперь Javalin принимает только локальные
соединения. Файрвольное правило для порта 25590 после этого не нужно
вообще.

Если по какой-то причине нужно оставить bind на `0.0.0.0` (например,
nginx живёт на другой машине), запрети порт снаружи в файрволе:

```bash
sudo ufw delete allow 25590/tcp
sudo ufw allow from <ip-nginx> to any port 25590 proto tcp
```
...
- Поставь Cloudflare перед Minecraft (через `proxy_protocol`) или
  fail2ban / crowdsec на уровне ОС, если нагрузка серьёзная. Готовый
  fail2ban-фильтр и инструкции по hardening на Ubuntu — в
  [`LINUX-HARDENING.ru.md`](LINUX-HARDENING.ru.md).
